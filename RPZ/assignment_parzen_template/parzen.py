...
def my_parzen(x, x_trn, h):
    ...
    p = [np.sum(norm.pdf(xi-x_trn,0,scale=h))/len(x_trn) for xi in x]
    return np.array(p)
...
def compute_Lh(itrn, itst, x, h):
    ...
    """
    Lh = float(np.sum(np.array([np.sum(np.log(my_parzen(x[i], x[j], h))) for i,j in zip(itst, itrn) ]))/10)
    return Lh
...
def classify_bayes_parzen(x_test, xA, xC, pA, pC, h_bestA, h_bestC):
    ...
    pos_A = my_parzen(x_test, xA, h_bestA)
    pos_C = my_parzen(x_test, xC, h_bestC)
    labels = 1*np.array(pos_A*pA < pos_C*pC)
    return labels
