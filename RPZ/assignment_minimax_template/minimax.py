...
def minimax_strategy_discrete(distribution1, distribution2):
    ...
    r = np.ravel(distribution1/distribution2)
    first_dist, second_dist = distribution1.ravel(),distribution2.ravel()
    r_sort = np.argsort(r)
    eps1, eps2 = np.zeros(r_sort.shape),np.zeros(r_sort.shape)
    max_error = 1
    for i in r_sort:
        eps1[i] = np.sum(first_dist[r_sort[:i+1]])
        eps2[i] = np.sum(second_dist[r_sort[i+1:]])
        err = max(np.sum(first_dist[r_sort[:i+1]]),np.sum(second_dist[r_sort[i+1:]]))
        if err < max_error:
            max_error = err
            opt_i = i
    eps1, eps2 = np.array(eps1), np.array(eps2)
    q = np.zeros(r_sort.shape)
    q[r_sort[:opt_i+1]] = 1
    q = q.reshape(distribution1.shape).astype(np.int)
...
def classify_discrete(imgs, q):
    ...
    x,y = compute_measurement_lr_discrete(imgs),compute_measurement_ul_discrete(imgs)
    x,y = x+10,y+10
...
def worst_risk_cont(distribution_A, distribution_B, true_A_prior):
    ...
    for i in np.linspace(0,1,101):
        distribution_A['Prior'] = i
        distribution_B['Prior'] = 1- i
        risk = bayes_risk_2normal(distribution_A,distribution_B, q)
        if risk > worst_risk:
            worst_risk = risk
    return worst_risk
...
def minimax_strategy_cont(distribution_A, distribution_B):
    ...
    """
    helper_function = lambda prior: worst_risk_cont(distribution_A, distribution_B, prior)
    p_A = opt.fminbound(helper_function,0,1,full_output=True)[0]
    distribution_A['Prior'] = p_A
    distribution_B['Prior'] = 1-p_A
    q = find_strategy_2normal(distribution_A,distribution_B)
...
def risk_fix_q_cont(distribution_A, distribution_B, distribution_A_priors, q):
    ...
    p_A = distribution_A_priors
    p_B = 1-p_A
    R0 = 0
    R1 = 0
    for i, choice in enumerate(list(q['decision'])):
        if choice == 0:
            if i == 0:
                R0 = p_B*norm.cdf(q['t1'], m_B, s_B)
            elif i == 1:
                R0 += (p_B*(norm.cdf(q['t2'], m_B, s_B))) - (p_B*(norm.cdf(q['t1'], m_B, s_B)))
            else:
                R0 += p_B*(1-norm.cdf(q['t2'], m_B, s_B))
        else:
            if i == 0:
                R1 = p_A*norm.cdf(q['t1'], m_A, s_A) 
            elif i == 1:
                R1 += (p_A*norm.cdf(q['t2'], m_A, s_A)) - (p_A*norm.cdf(q['t1'], m_A, s_A))
            else:
                R1 += p_A*(1-norm.cdf(q['t2'], m_A, s_A))   
    risks = R0+R1
    return risks
...
def classification_error(predictions, labels):
    ...
    """
    error = np.sum(predictions!=labels)/len(predictions)
    return float(error)
...
def find_strategy_2normal(distribution_A, distribution_B):
    ...
    if p_A < eps:
        q['t1'] = -np.inf
        q['t2'] = np.inf
        q['decision'] = np.array([1,1,1]).astype(np.int32)
    elif p_B < eps:
        q['t1'] = -np.inf
        q['t2'] = np.inf
        q['decision'] = np.array([0, 0, 0]).astype(np.int32)
    else:
        a = (1/(2*s_B**2)) - (1/(2*s_A**2))
        b = (2*m_A/(2*s_A**2)) - (2*m_B/(2*s_B**2))
        c = (m_B**2/(2*s_B**2))-(m_A**2/(2*s_A**2))+np.log(p_A/p_B) + np.log(s_B/s_A)
        if a == 0:
            ...
            if b == 0:
                ...
                q['t2'] = float(t)
                if b > 0:
                    q['decision'] = np.array([1, 0, 0]).astype(np.int32)
                else:
                    q['decision'] = np.array([0, 0, 1]).astype(np.int32)
        else:
            # quadratic equation
            D = b**2 - 4*a*c
            if D > 0:
                ...
                if t1 < t2:
                    ...
                    q['t2'] = float(t1)
                if a < 0:
                    q['decision'] = np.array([1,0,1]).astype(np.int32)
                else:
                    q['decision'] = np.array([0,1,0]).astype(np.int32)
            elif D == 0:
                t = -b/(2*a)
                q['t1'] = float(t)
                q['t2'] = float(t)
...
def bayes_risk_2normal(distribution_A, distribution_B, q):
    ...
    for i, choice in enumerate(list(q['decision'])):
        if choice == 0:
            if i == 0:
                R = p_B*norm.cdf(q['t1'], m_B, s_B)
            elif i == 1:
                R += (p_B*(norm.cdf(q['t2'], m_B, s_B))) - (p_B*(norm.cdf(q['t1'], m_B, s_B)))
            else:
                R += p_B*(1-norm.cdf(q['t2'], m_B, s_B))
        else:
            if i == 0:
                R = p_A*norm.cdf(q['t1'], m_A, s_A) 
            elif i == 1:
                R += (p_A*norm.cdf(q['t2'], m_A, s_A)) - (p_A*norm.cdf(q['t1'], m_A, s_A))
            else:
                R += p_A*(1-norm.cdf(q['t2'], m_A, s_A))   
    return float(R)
...
def classify_2normal(measurements, q):
    ...
    """
    a = np.where(measurements <= q['t1'])
    b = np.where((q['t1'] < measurements) & (measurements <= q['t2']))
    c = np.where(measurements > q['t2'])
    output = np.zeros(measurements.shape)
    output[a] = 0
    output[b] = 1
    output[c] = 2
    strategy = q['decision']
