...
class Linear(Layer):
    ...
    def backward(self, dL_wrt_output):
        ...
        """
        dL_wrt_x = np.matmul(dL_wrt_output, self.W.T)
        # self.dL_wrt_W =[]
        # for i in range(dL_wrt_output.shape[0]):
        #     mult = np.matmul(self.x[i].reshape(-1,1),dL_wrt_output[i].reshape(-1,1).transpose())
        #     self.dL_wrt_W.append(mult)
        # self.dL_wrt_W = np.array(self.dL_wrt_W)
        self.dL_wrt_W = np.swapaxes(np.multiply(self.x[:,np.newaxis, :], dL_wrt_output[:,:,np.newaxis]), 1,2)
        self.dL_wrt_b = dL_wrt_output.reshape((dL_wrt_output.shape[0],1,dL_wrt_output.shape[1]))
        return dL_wrt_x
...
def main():
    ...
    model = [Linear(D, 40), ReLU(), Linear(40, 20),ReLU(), Linear(20, 1), Sigmoid()]
    # model = [Linear(D, 30), Sigmoid(), Linear(30, 1), Sigmoid()]
    # model = [Linear(D, 1), Sigmoid()]
    trn_head = SE()
