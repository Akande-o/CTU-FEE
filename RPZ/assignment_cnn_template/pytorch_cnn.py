...
class SimpleCNN(nn.Module):
    def __init__(self):
        ...
        self.conv = nn.Conv2d(in_channels=1,
                              ...
                              stride=2,
                              padding=1)
        self.fc = nn.Linear(in_features=28 * 28 * 10//(4*4) ,
                            out_features=10)
...
class MyNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.first_conv_layer = nn.Conv2d(1, 16,kernel_size=3, stride=2,padding=1)
        self.second_conv_layer = nn.Conv2d(16, 32,kernel_size=3, stride=2,padding=1)
        self.second_linear_layer = nn.Linear(256, 30)
        self.third_conv_layer = nn.Conv2d(32, 64, kernel_size=3, stride=2,padding=1)
        self.third_linear_layer = nn.Linear(30, 10)
    def forward(self, x):
...
def classify(model, x):
    ...
    """
    value, labels = torch.max(model(x), dim=1)
    return labels
