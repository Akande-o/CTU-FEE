...
def ml_estim_normal(x):
    ...
    """
    mu = np.mean(x)
    var = np.sum(np.square(x-mu))/len(x)
...
def map_estim_normal(x, mu0, nu, alpha, beta):
    ...
    """
    mu = (np.sum(x) + nu*mu0)/(len(x)+ nu)
    var = (np.sum(np.square(x-mu)) + 2*beta + nu*(mu0-mu)**2)/(len(x)+3+2*alpha)
    return float(mu), float(var)
...
def bayes_posterior_params_normal(x, prior_mu0, prior_nu, prior_alpha, prior_beta):
    ...
    """
    mu0  = (prior_nu*prior_mu0 + np.sum(x))/(prior_nu+len(x))
    nu = prior_nu + len(x)
    alpha = prior_alpha + len(x)/2
    beta = prior_beta + np.sum(np.square(x))/2 + prior_nu*prior_mu0**2/2 - ((prior_nu*prior_mu0 + np.sum(x))**2/(2*(prior_nu + len(x))))
...
def bayes_estim_pdf_normal(x_test, x,
    ...
    alpha_tilda = float(alpha + 1/2)
    nu_tilda = float(nu + 1)
    beta_tilda = (np.square(x_test)/2) + beta + ((nu*mu0**2)/2) - ((np.square(nu*mu0 +x_test))/(2*(nu + 1)))
    pdf = (1 / np.sqrt(2 * np.pi)) * (np.sqrt(nu) * beta**alpha / (np.sqrt(nu_tilda) * beta_tilda**alpha_tilda)) * (spec.gamma(alpha_tilda) / spec.gamma(alpha))
    return pdf
...
def bayes_estim_categorical(counts, alphas):
    ...
    :return:        pk - estimated categorical distribution parameters, numpy array (n, )
    """
    pk = (counts+alphas)/np.sum(counts[:6] + alphas[:6])
    return pk
...
def mle_Bayes_classif(x_test, x_train_A, x_train_C):
    ...
    """
    DA, DC = {}, {}
    a_mean, a_var = ml_estim_normal(x_train_A)
    c_mean, c_var = ml_estim_normal(x_train_C)
    DA['Mean'], DA['Sigma'] = a_mean, float(np.sqrt(a_var))
    DC['Mean'], DC['Sigma'] = c_mean, float(np.sqrt(c_var))
    DA['Prior'], DC['Prior'] = len(x_train_A)/(len(x_train_A)+len(x_train_C)), len(x_train_C)/(len(x_train_A)+len(x_train_C))
...
def map_Bayes_classif(x_test, x_train_A, x_train_C,
    ...
    """
    DA, DC = {}, {}
    a_mean, a_var = map_estim_normal(x_train_A,mu0_A,nu_A, alpha_A,beta_A)
    c_mean, c_var = map_estim_normal(x_train_C,mu0_C,nu_C,alpha_C,beta_C)
    DA['Mean'], DA['Sigma'] = a_mean, float(np.sqrt(a_var))
    DC['Mean'], DC['Sigma'] = c_mean, float(np.sqrt(c_var))
    DA['Prior'], DC['Prior'] = len(x_train_A)/(len(x_train_A)+len(x_train_C)), len(x_train_C)/(len(x_train_A)+len(x_train_C))
    q = find_strategy_2normal(DA, DC)
...
def bayes_Bayes_classif(x_test, x_train_A, x_train_C,
    ...
    """
    posterior_A = bayes_estim_pdf_normal(x_test,x_train_A,mu0_A,nu_A,alpha_A, beta_A)
    posterior_C = bayes_estim_pdf_normal(x_test,x_train_C,mu0_C,nu_A,alpha_C, beta_C)
    p_A, p_C = len(x_train_A)/(len(x_train_A)+len(x_train_C)), len(x_train_C)/(len(x_train_A)+len(x_train_C))
    labels = np.array(p_A*posterior_A <= p_C*posterior_C,dtype="int32")
    return labels
