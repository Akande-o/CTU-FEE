...
def logistic_loss(X, y, w):
    ...
    """
    z = X*y
    E = np.sum(np.log(1 + np.exp(-np.matmul(w,z))))/len(y)
    return E
...
def logistic_loss_gradient(X, y, w):
    ...
    """
    g = -np.sum(((X*y)/(1 + np.exp(np.matmul(w, (X*y))))),axis = 1)/len(y)
    return g
...
def logistic_loss_gradient_descent(X, y, w_init, epsilon):
    ...
    """
    wt = [np.zeros(w_init.shape)]
    Et = []
    w = w_init
    wt.append(w_init)
    step_size = 1.0
    E,g = logistic_loss(X,y, w), logistic_loss_gradient(X,y,w)
    Et.append(E)
    while np.linalg.norm(wt[-1]-wt[-2]) >= epsilon:
        E_new, g_new = logistic_loss(X,y,w-step_size*g), logistic_loss_gradient(X,y,w-step_size*g)
        if E_new < E:
            w = w- step_size*g
            g = g_new
            E = E_new
            ...
            step_size /= 2
    wt.pop(0)
    return w, np.array(wt).transpose(), np.array(Et)
